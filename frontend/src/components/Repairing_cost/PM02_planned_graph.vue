<template>
    <div>
      <v-card>
        <v-card-title>PM02 Planned cost</v-card-title>
        <div id="rpcPM02P"></div>
      </v-card>
    </div>
  </template>
    ...
    mounted() {
      ...
      const getRepairingCostData = async () => {
        try {
          ...
          for (const plantData of repairingCostData) {
            const plannedPM02Data = plantData.PlannedPM02;
            // monthとcostの列だけを抽出して新しいデータ形式に変換
            const transformedData = plannedPM02Data.map(entry => ({ x: entry.month, y: entry.cost }));
            // Vueのdataに追加
      ...
      getRepairingCostData()
        .then(() => {
          ...
          Plotly.newPlot('rpcPM02P', plotData, layout);
        })
        .catch(error => {
