...
<script>
  ...
  const URL = "http://127.0.0.1:8000/api/repairingCost/PPM02ByCompany/?format=json";
    ...
    methods: {
      getDataAxios() {
        axios.get(URL, {
          ...
          .then(response => {
            let plannedCostData = response.data;
            ...
            if (userCompanyCode) {
              plannedCostData = plannedCostData.filter(companyData => companyData.companyCode === userCompanyCode);
            }
  
            //データ抽出
            const months = ["year","jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec", "commitment",];
            const tableData = plannedCostData.flatMap(companyData =>
