<template>
    <div>
        ...
        <div class="surface-section px-4 py-8 md:px-6 lg:px-8">
            <div class="py-4 text-center md:text-left border-round bg-cover h-20rem lg:h-30rem flex flex-column justify-content-end mb-5" :style="{ backgroundImage: `url(${Schedule_Explanation})` }">
                <div class="grid">
                    <div class="col lg:col-offset-2 px-5">
                        <div class="text-5xl font-bold text-black">Visualization of Repair Costs</div>
            ...
            <div class="grid">
                ...
                <div class="col-12 lg:col lg:px-5">
                    ...
                    <div class="line-height-3 text-xl text-900 mb-5">
                        ここでは、ガントチャートの使い方について説明します。ガントチャートは、上部に工場ラインのスケジュール（プラントスケジュール）を表示させ、下部にメンテナンスタスクのスケジュール（タスクスケジュール）を表示させてます。
                        プラントメンテナンスは、ほとんどが安全性の観点からシャットダウン時での作業になるので、プラントスケジュールとタスクスケジュールと照合し、全体のスケジュールの調整を行ってください。
                        ガントチャートは、TaskListに各タスクを記入して頂ければ、ガントチャート上で、そのスケジュールが反映されます。
                        また、直接タスク名の変更や、ガントバーをマウス操作で調整することで、スケジュールの変更は可能です。
                        その場合、マウス操作後に、忘れずに保存ボタンを押してください。保存が成功した場合は、TaskListやWorking Historyの値も同様に変更されます。
                        

                        ガントチャートは、毎月ごとの表示を行っています。
                        今後、毎週や毎年事にタブをクリックすることで変更予定です。
                        また、各業者に対しても同様にcompanyCodeとパスを連絡することで、ガントチャートは社外での第3とのガントチャートの共有が可能です。
                        その場合、タスク内容については、メインユーザーと業者だけでの編集が可能になります。
  ...
  import Schedule_Explanation from '@/assets/Schedule_Explanation.png';
