...
onMounted(() => {
  ...
  (newTasks, oldTasks) => {
      newTasks.forEach((task, index) => {
          // startDateやendDateがundefinedでないか確認してから比較
          if (task.startDate && task.endDate) {
              if (
                  task.startDate !== oldTasks[index]?.startDate ||
                  task.endDate !== oldTasks[index]?.endDate
              ) {
                  updateChartContent(); // フォームで日付が変更されたときにチャートを再描画
              }
          }
      });
  },
