...
export default {
    ...
    data() {
        return {
            ...
            dragOffset: 0,
            width: 1200,
            margin: { top: 50, right: 50, bottom: 10, left: 120 },
            plotHeight: 500,
            histogramHeight: 300,
    ...
    methods: {
        async fetchEventData() {
            ...
            try {
                ...
                // データの整形
                const eventDataPPM02 = responsePPM02.data.flatMap((item) =>
                    item.taskListPPM02.map((task) => ({
                ...
                const eventDataPPM03 = responsePPM03.data.flatMap((item) =>
                    item.taskListPPM03.map((task) => ({
                        date: new Date(task.nextEventDate),
        ...
        async fetchFaultData() {
            ...
            try {
                ...
                // データの整形
                this.faultData = response.data.flatMap((item) =>
                    item.troubleHistory.map((task) => ({
        ...
        async fetchPredictionData() {
            ...
            try {
                ...
                // データの整形
                this.predictionData = response.data.flatMap((item) =>
                    item.failurePredictionPoint.map((task) => ({
                        date: new Date(task.date),
                        ceListNo: task.ceListNo,
                        equipmentNo: parseInt(task.ceListNo), // ここでceListNoを整数に変換
                        pmType: task.pmType
        ...
        createPlotGraph() {
            ...
            const svg = d3
                ...
                .style('background', '#f0f0f0') // グラフ全体の背景色を設定（薄いグレー）
                .append('g')
                .attr('transform', `translate(${margin.left},${margin.top})`);

            // プロット領域の背景色（白）を設定
            svg.append('rect').attr('width', graphWidth).attr('height', height).attr('x', 0).attr('y', 0).style('fill', 'white'); // プロット領域のみ白にする
            ...
            this.yScale = d3
                .scaleTime()
                .domain([new Date(currentDate.getFullYear(), currentDate.getMonth() - 12, 1), new Date(currentDate.getFullYear(), currentDate.getMonth() + 12, 1)])
                .range([height, 0]);
            ...
            // y軸グリッド線の追加
            svg.append('g').attr('class', 'grid').call(d3.axisLeft(this.yScale).tickSize(-graphWidth).tickFormat('')).selectAll('line').style('stroke', 'lightgrey').style('stroke-opacity', '0.7').style('shape-rendering', 'crispEdges');

            // x軸ラベルの追加
            svg.append('text')
                .attr('x', graphWidth / 2)
                .attr('y', height + margin.bottom + 20) // margin.bottomに依存させて調整
                .attr('text-anchor', 'middle')
            ...
            filteredFaultMap.forEach((d) => {
                ...
                if (d.typeOfTask === 'PM04') {
                    // 雷の形状を赤色にしてプロット
                    group.append('path').attr('d', lightningPath).attr('transform', `scale(0.5) translate(-10,-20)`).style('fill', 'red');
                } else if (d.typeOfTask === 'PM03') {
                    sizesPM03.forEach((size, i) => {
                        group.append('circle').attr('r', size).style('fill', colorsPM03[i]).style('stroke', 'black').style('stroke-width', 1);
                    });
                }
            ...
            // 予測データのプロット（的の形状）
            this.predictionPaths = svg
                .selectAll('g.prediction')
            ...
            this.predictionBands = svg
                .selectAll('rect.predictionBand')
                .data(filteredPredictionData)
                .enter()
                .append('rect')
            ...
            // ドラッグ機能をSVGに追加
            svg.append('rect').attr('width', graphWidth).attr('height', height).style('fill', 'none').style('pointer-events', 'all').call(drag);
        },
        ...
        createHistogramGraph() {
            ...
            const svg = d3
                ...
                .style('background', '#f0f0f0') // グラフ全体の背景色を設定
                .append('g')
                .attr('transform', `translate(${margin.left},${margin.top})`);
            ...
            const yScale = d3
                .scaleLinear()
                .domain([0, d3.max([...histogramData.values(), ...faultHistogramData.values()])])
                .range([height, 0]);
            ...
            // y軸グリッド線の追加
            svg.append('g').attr('class', 'grid').call(d3.axisLeft(yScale).tickSize(-graphWidth).tickFormat('')).selectAll('line').style('stroke', 'lightgrey').style('stroke-opacity', '0.7').style('shape-rendering', 'crispEdges');
        ...
        createCostHistogramGraph() {
            ...
            // 合計コストデータの作成
            const eventCostData = Array.from(
                d3.rollup(
                    filteredData,
            ...
            const faultCostData = Array.from(
                d3.rollup(
                    filteredFaultMap,
                    (v) => d3.sum(v, (d) => d.cost),
                    (d) => d.equipmentNo
            ...
            this.faultCostBars = svg
                .selectAll('rect.faultCost')
                .data(faultCostData)
                .enter()
                .append('rect')
        ...
        createPlotLegend() {
            ...
            const legend = d3.select(this.$refs.plotLegend).append('svg').attr('width', 600).attr('height', 50); // 高さを少し大きめに変更
            ...
            legendItem
                .append('path')
                .filter((d) => d.type === 'PM04')
                .attr('d', 'M10 0 L20 20 L0 20 L10 40 L30 10 L10 10 Z')
                .attr('transform', 'scale(0.5) translate(10,10)')
                .style('fill', 'red');

            // PM03 同心円の形状
            legendItem
                .filter((d) => d.type === 'PM03')
                .each(function () {
        ...
        createHistogramLegend() {
            ...
            const legend = d3.select(this.$refs.histogramLegend).append('svg').attr('width', 400).attr('height', 30);
        ...
        createCostLegend() {
            ...
            const legend = d3.select(this.$refs.costLegend).append('svg').attr('width', 400).attr('height', 30);
        ...
        updateAxes() {
            ...
            const newXScale = this.xScale
                .copy()
                .domain(newDomain)
                .range([currentOffset, currentOffset + graphWidth]);
            const newHistXScale = this.histXScale
                .copy()
                .domain(newDomain)
                .range([currentOffset, currentOffset + graphWidth]);
            const newCostHistXScale = this.costHistXScale
                .copy()
                .domain(newDomain)
                .range([currentOffset, currentOffset + graphWidth]);
