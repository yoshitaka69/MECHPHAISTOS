...
const generatePDF = () => {
    ...
    const opt = {
        margin: 10,
        filename: 'Safety_Confirmation_Certificate.pdf',
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale: 2 },
        jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' },
        pagebreak: { mode: ['css'] }  // CSSでページ区切りを有効にするオプション
    };
...
.request-form-title, .confirmation-title {
    text-align: center;
    font-weight: bold;
    margin-bottom: 8px;
    width: 100%;
...
.form-grid-smaller, .grid-two-columns, .grid-five-columns {
    display: grid;
    gap: 16px;
    width: 100%;
}
...
.separator-full {
    border-bottom: 1px solid #000;
    width: calc(100% - 20mm); /* A4の横幅一杯に広げる */
}
...
@media print {
    .confirmation-page {
        page-break-after: always; /* 各ページをA4の範囲内に収める */
    }

    .page-break {
        page-break-before: always; /* ページ区切りを追加 */
    }
...
@media screen and (max-width: 480px) {
    .grid-two-columns, .grid-five-columns {
