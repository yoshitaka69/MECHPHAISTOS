  ...
  <div class="container">
    <div class="chart-section">
      ...
      <div class="pointer-position">
        Pointer Position: x={{ controlPoint.x.toFixed(2) }}, y={{ controlPoint.y.toFixed(2) }}
      </div>
      <div class="controls">
      ...
      <div class="history-buttons">
        <div v-for="(item, index) in history" :key="item.timestamp" class="history-item">
          <span @click.stop="toggleFavorite(index)" class="star-container">
            <i :class="favoriteHistory === index ? 'pi pi-star-fill' : 'pi pi-star'" class="star-icon"></i>
          </span>
...
export default {
  ...
  components: {
    Button, // PrimeVue Button コンポーネントをインポート
  },
  props: {
    inputData: {
      type: Array,
  ...
  data() {
    return {
      ...
      selectedSetting: 1,
      history: [],
      favoriteHistory: null, // どの履歴が★かを管理する
    };
  },
  watch: {
    inputData: {
    ...
    async loadHistory() {
      ...
      try {
        ...
        this.history = response.data.map((item, index) => {
          ...
          if (index === 0) {
            this.favoriteHistory = 0; // 初回ロード時に最初の履歴を★に設定
          }
          return item;
        });
        console.log('History loaded:', this.history);

        // モーダルが開かれたときに★がついている履歴データをロード
        if (this.favoriteHistory !== null) {
          this.setControlPoint(this.history[this.favoriteHistory]);
        }
      } catch (error) {
