...
export default {
  ...
  mounted() {
    ...
    console.log('Initial emittedData:', this.emittedData);
    this.settingsOptions.forEach(setting => {
      this.drawChart(setting.value);
      this.setupResizeObserver(setting.value);
    });
    this.processEmittedData();
  },
  watch: {
    emittedData(newVal, oldVal) {
      console.log('emittedData changed:', newVal);
      this.processEmittedData();
    }
  },
  data() {
  ...
  methods: {
    setupResizeObserver(setting) {
      const container = this.$refs[`chartContainer-${setting}`];
      if (container && container.length) {
        this.resizeObservers[setting] = new ResizeObserver(() => {
          this.drawChart(setting);
        });
        this.resizeObservers[setting].observe(container[0]);
      }
    },
    drawChart(setting) {
      const container = this.$refs[`chartContainer-${setting}`];
    ...
    processEmittedData() {
      this.emittedData.forEach(row => {
        if (row.levelSetValue && row.mttr && row.possibilityOfContinuousProduction) {
          let riskMatrix;
          if (row.levelSetValue === 'Low') {
            riskMatrix = this.riskMatrixInitial[0].risk;
          } else if (row.levelSetValue === 'Middle') {
            riskMatrix = this.riskMatrixInitial[1].risk;
          } else if (row.levelSetValue === 'High') {
            riskMatrix = this.riskMatrixInitial[2].risk;
          }

          if (riskMatrix) {
            const mttrIndex = this.getMttrIndex(row.mttr);
            const possibilityIndex = this.getPossibilityIndex(row.possibilityOfContinuousProduction);

            const riskText = this.mapRiskText(riskMatrix[possibilityIndex]);
            console.log(`RiskMatrix value for Level: ${row.levelSetValue}, MTTR: ${row.mttr}, Possibility: ${row.possibilityOfContinuousProduction} is ${riskText}`);

            this.$emit('update-impact-for-production', {
              index: this.emittedData.indexOf(row),
              impactForProduction: riskText
            });
          }
        }
      });
    ...
    getPossibilityIndex(possibility) {
      return 5 - possibility; // Assuming possibility values are between 1 to 5
    },
