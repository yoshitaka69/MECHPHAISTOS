<template>
  <div class="chart-container">
      ...
      <svg ref="chart" preserveAspectRatio="xMinYMin meet" viewBox="0 0 800 400" class="chart-svg"></svg>
      ...
      <a href="#" @click.prevent="showModal = true">詳細を表示</a>
      <!-- リンクを追加 -->
...
import JohnMoubrayReliability from '@/components/Critical_equipment_list/CeListDetailPage/JohnMoubrayReliability.vue'; // モーダル用コンポーネントをインポート
...
export default {
  ...
  methods: {
      ...
      renderChart() {
          ...
          const y = d3
              .scaleLinear()
              .domain([0, d3.max(this.bathtub_curve)])
          ...
          // グラフを描画
          svg.append('path').datum(this.bathtub_curve).attr('fill', 'none').attr('stroke', 'black').attr('stroke-width', 2).attr('d', line).attr('d', line);
          ...
          // 領域の凡例
          const legend = svg.append('g').attr('transform', `translate(${width + margin.right / 2},${margin.top})`);

          legend.append('rect').attr('x', 0).attr('y', 0).attr('width', 10).attr('height', 10).attr('fill', 'lightblue').attr('opacity', 0.3);

          legend.append('text').attr('x', 20).attr('y', 10).style('font-size', '12px').text('初期故障期');

          legend.append('rect').attr('x', 0).attr('y', 20).attr('width', 10).attr('height', 10).attr('fill', 'lightgreen').attr('opacity', 0.3);

          legend.append('text').attr('x', 20).attr('y', 30).style('font-size', '12px').text('偶発故障期');

          legend.append('rect').attr('x', 0).attr('y', 40).attr('width', 10).attr('height', 10).attr('fill', 'lightcoral').attr('opacity', 0.3);

          legend.append('text').attr('x', 20).attr('y', 50).style('font-size', '12px').text('摩耗故障期');

          legend.append('rect').attr('x', 0).attr('y', 60).attr('width', 10).attr('height', 10).attr('fill', 'black');

          legend.append('text').attr('x', 20).attr('y', 70).style('font-size', '12px').text('バスタブ曲線');
      }
  }
};
</script>
