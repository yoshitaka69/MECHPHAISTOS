...
export default {
    ...
    mounted() {
        const getSustainabilityData = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/sustainability/wellWaterByCompany/?format=json');
                ...
                for (const companyData of sustainabilityData) {
                    for (const plantData of companyData.WellWaterList) {
                        ...
                        const wellWaterData = plantData.WellWater; // ここで co2Data を定義
