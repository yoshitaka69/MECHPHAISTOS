<template>
  <div id="gantt-container" class="p-4 space-y-8">
    <!-- 上部のガントチャート -->
    <div class="gantt-chart-wrapper top-chart bg-white shadow-md rounded p-4">
      <div id="gantt-chart-container-1" class="overflow-auto">
        <!-- ヘッダー領域 -->
        <div class="flex">
          <div id="top-header" class="z-20 sticky left-0 bg-green-100" :style="{ width: `${topTaskFormWidth}px` }">
            <div class="flex">
              <div class="py-2 px-2 border-r border-black text-sm" style="width: 30px;">✔</div>
              <!-- チェックボックス列のヘッダー -->
              <div class="py-2 px-2 border-r border-black text-sm" :style="{ width: `${inputWidths.name}px` }">Plant</div>
              <div class="py-2 text-center border-r border-black text-sm" :style="{ width: `${inputWidths.startDate}px` }">
                開始日
              </div>
              <div class="py-2 text-center border-r border-black text-sm" :style="{ width: `${inputWidths.endDate}px` }">期限日</div>
              <div
                v-for="column in topExtraColumns"
                :key="column.key"
                class="text-center border-r border-black text-sm"
                :style="{ width: `${extraColumnWidth}px` }"
              >
                {{ column.label }}
              </div>
            </div>
          ...
          <!-- カレンダー日付領域 -->
          <div id="top-calendar" class="flex" :style="{ width: `${topViewWidth}px` }">
        ...
        <div class="flex">
          <!-- タスク入力領域 -->
          <div id="top-task-inputs" :style="{ width: `${topTaskFormWidth}px` }">
            <div v-for="task in topTaskRows" :key="task.id" class="task-row top-task-row flex">
              <div class="bg-green-100 z-10 flex sticky left-0" :style="{ width: `${topTaskFormWidth}px` }">
                ...
                <input
                  type="checkbox"
                  class="py-2 h-full border-r border-black"
                  style="width: 30px;"
                ...
                <div
                  v-for="column in topExtraColumns"
                  :key="column.key"
                  class="bg-green-100 text-center border-r border-black text-sm"
                  :style="{ width: `${extraColumnWidth}px` }"
                >
                  <input
          ...
          <div id="top-gantt-bars" class="relative" :style="{ width: `${topViewWidth}px` }">
            ...
            <div v-for="task in topTaskRows" :key="task.id" class="h-10 flex py-2 will-change-transform cursor-pointer">
              ...
              <div
                ...
                @mousedown.stop="onMouseDown_ResizeStart($event, task, 'left', topTasks)"
              ></div>
              <div
                class="w-2 bg-yellow-200 rounded-r-lg cursor-col-resize"
                @mousedown.stop="onMouseDown_ResizeStart($event, task, 'right', topTasks)"
              ></div>
            </div>
          </div>
        </div>
    ...
    <!-- 下部のガントチャート -->
    <div class="gantt-chart-wrapper bottom-chart bg-white shadow-md rounded p-4">
      <div id="gantt-chart-container-2" class="overflow-auto">
        <!-- ヘッダー領域 -->
        <div class="flex">
          <div id="bottom-header" class="z-20 sticky left-0 bg-blue-100" :style="{ width: `${bottomTaskFormWidth}px` }">
            <div class="flex">
              <div class="py-2 px-2 border-r border-black text-sm" style="width: 30px;">✔</div>
              <!-- チェックボックス列のヘッダー -->
              <div class="py-2 px-2 border-r border-black text-sm" :style="{ width: `${inputWidths.name}px` }">タスク</div>
              <div class="py-2 text-center border-r border-black text-sm" :style="{ width: `${inputWidths.sample}px` }">
                サンプル
              </div>
              <div class="py-2 text-center border-r border-black text-sm" :style="{ width: `${inputWidths.pmType}px` }">
                PM type
              </div>
              <div class="py-2 text-center border-r border-black text-sm" :style="{ width: `${inputWidths.assignee}px` }">
                担当者
              </div>
              <div class="py-2 text-center border-r border-black text-sm" :style="{ width: `${inputWidths.startDate}px` }">
                開始日
              </div>
              <div class="py-2 text-center border-r border-black text-sm" :style="{ width: `${inputWidths.endDate}px` }">
                期限日
              </div>
              <div
                v-for="column in bottomExtraColumns"
                :key="column.key"
                class="text-center border-r border-black text-sm"
                :style="{ width: `${extraColumnWidth}px` }"
              >
          ...
          <!-- カレンダー日付領域 -->
          <div id="bottom-calendar" class="flex" :style="{ width: `${bottomViewWidth}px` }">
            <template v-for="month in calendars" :key="month.title">
              <div class="border-b border-black bg-white" :style="{ minWidth: `${blockWidth * month.days.length}px` }">
                <div class="border-b border-r border-black px-2 text-xs">{{ month.title }}</div>
                <div class="flex">
        ...
        <div class="flex">
          <!-- タスク入力領域 -->
          <div id="bottom-task-inputs" :style="{ width: `${bottomTaskFormWidth}px` }">
            <div v-for="task in bottomTaskRows" :key="task.id" class="task-row bottom-task-row flex">
              <div class="bg-blue-100 z-10 flex sticky left-0" :style="{ width: `${bottomTaskFormWidth}px` }">
                ...
                <div
                  v-for="column in bottomExtraColumns"
                  :key="column.key"
                  class="bg-blue-100 text-center border-r border-black text-sm"
                  :style="{ width: `${extraColumnWidth}px` }"
                >
                  <input
                    class="py-2 h-full text-center"
          ...
          <div id="bottom-gantt-bars" class="relative" :style="{ width: `${bottomViewWidth}px` }">
            ...
            <div v-for="task in bottomTaskRows" :key="task.id" class="h-10 flex py-2 will-change-transform cursor-pointer">
              ...
              <div
                class="w-2 bg-yellow-200 rounded-l-lg cursor-col-resize"
                @mousedown.stop="onMouseDown_ResizeStart($event, task, 'left', tasks)"
              ></div>
              <div
                class="w-2 bg-yellow-200 rounded-r-lg cursor-col-resize"
                @mousedown.stop="onMouseDown_ResizeStart($event, task, 'right', tasks)"
              ></div>
            </div>
          </div>
        </div>
...
function removeSelectedTopTasks() {
  ...
  topTasks.value = topTasks.value.filter((task) => {
    console.log(`Task ID: ${task.id}, Selected: ${task.selected}`);
    return !task.selected; // selected が true のタスクを削除
  });
...
// チェックボックスがクリックされた後の状態を確認するためにログを追加
function toggleSelection(task) {
  console.log(`Before Toggle - Task ID: ${task.id}, Selected: ${task.selected}`);
  task.selected = !task.selected;
  console.log(`After Toggle - Task ID: ${task.id}, Selected: ${task.selected}`);
}
...
#gantt-container {
  min-width: 1000px; /* 全体のコンテナの最小幅を指定 */
  min-height: 600px; /* 全体のコンテナの最小高さを指定 */
  max-width: 100%; /* コンテナの最大幅を100%に設定 */
}

.bottom-chart {
  margin-top: 24px; /* 上下のガントチャート間に24pxの余白を追加 */
}
