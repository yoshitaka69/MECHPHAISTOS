        ...
        <div class="controls">
            <div class="settings" v-for="(setting, index) in settingsOptions" :key="setting.value">
                ...
                <div class="history-container">
                    <h4>History</h4>
                    <ul>
                        <li v-for="history in historyData[setting.value]" :key="history.timestamp" @click="loadHistoryPoint(setting.value, history)">{{ formatTimestamp(history.timestamp) }}: x={{ history.x }}, y={{ history.y }}</li>
                    </ul>
                </div>
            </div>
        </div>
