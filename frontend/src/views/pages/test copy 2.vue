    ...
    calculateTotalCostSums() {
        ...
        hotInstance.getData().forEach((row, rowIndex) => {
            for (let i = 0; i < 11; i++) {  // この年から10年後までの各年について
                ...
                if (row[checkboxColumnIndex]) {  // チェックボックスがtrueかどうか
                    const totalCost = parseFloat(row[6]);  // TotalCostは6番目の列
                    if (!isNaN(totalCost)) {
                        sums[i] += totalCost;  // 対応する年の合計に加算
                        console.log(`Row ${rowIndex + 1}: Added ${totalCost} to year ${currentYear + i} sum, new total: ${sums[i]}`);
                    } else {
                        console.log(`Row ${rowIndex + 1}: Invalid TotalCost at column 6, received: '${row[6]}'`);
                    }
                }
            }
        });
