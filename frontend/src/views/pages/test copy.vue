...
const Simulation_table = defineComponent({
  ...
  methods: {
    ...
    getDataAxios() {
      ...
      console.log('Fetching data from:', url); // デバッグ用にURLを出力
      ...
      return axios
        ...
        .then((response) => {
          console.log('API response received:', response.data); // デバッグ用にAPIレスポンスを出力

          const taskListData = response.data.flatMap((companyData) => companyData.taskList);
          console.log('Fetched Task List Data:', taskListData); // 取得したタスクリストデータを出力

          const blankRows = Array.from({ length: 20 }, () => ({}));
          const newData = taskListData.concat(blankRows);
          console.log('New data with blank rows:', newData); // タスクリストと空行を追加したデータを出力

          this.$refs.hotTableComponent.hotInstance.loadData(newData);
          console.log('Data loaded into Handsontable.'); // Handsontableにデータがロードされたか確認
        ...
        .catch((error) => {
          if (error.response) {
            ...
            console.error('No response from API. Request:', error.request);
          } else {
            // それ以外のエラー
    ...
    mounted() {
      console.log('Component mounted'); // コンポーネントがマウントされた時点でログを出力
      this.getDataAxios();
    },
