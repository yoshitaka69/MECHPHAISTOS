    ...
    <div id="app" v-if="form && calendarData.calendars">
        ...
        <div id="gantt-content" style="display: flex">
            ...
            <div id="gantt-calendar" class="overflow-x-scroll overflow-y-hidden border-l" :style="`width:${calendarViewWidth}px`" ref="calendar">
                ...
                <!--タスクバースタイル-->
                <div id="gantt-bar-area" :style="{ position: 'relative', width: calendarViewWidth + 'px', height: calendarViewHeight + 'px', minHeight: '50px' }">
...
export default {
    ...
    setup() {
        ...
        const taskBars = computed(() => {
            console.log('Calculating task bars');
            return tasks.value
                .map((task) => {
                    ...
                    if (isNaN(left) || isNaN(width)) {
                        console.error('Calculation error:', { left, width });
                        return null;
                    }

                    console.log(`Task bar for ${task.name}: left=${left}px, width=${width}px`);
                    return {
                        task,
                        style: {
                            position: 'absolute',
