    ...
    <div>
      <div class="controls">
        <div class="settings" v-for="(setting, index) in settingsOptions" :key="setting.value">
          ...
          <div class="history">
            <h4>History</h4>
            <button v-for="record in history[setting.value]" :key="record.timestamp" @click="setControlPoint(record, setting.value)">
  ...
  export default {
    ...
    mounted() {
      this.settingsOptions.forEach(setting => {
        ...
        this.loadHistory(setting.value);
      });
    ...
    methods: {
      ...
      saveControlPoint(setting) {
        ...
        const payload = {
          ...
          levelSetValue: this.getSettingName(setting),
        ...
        axios.post('http://127.0.0.1:8000/api/ceList/risk_matrix_impact/', payload)
          .then(response => {
            console.log('Response data:', response.data);
            this.loadHistory(setting);
          })
          .catch(error => {
      ...
      getCompanyCode() {
        ...
        return userStore.companyCode || '';
      },
      loadHistory(setting) {
        const companyCode = this.getCompanyCode();
        const levelSetValue = this.getSettingName(setting);
        axios.get(`http://127.0.0.1:8000/api/ceList/risk_matrix_impact/${companyCode}/${levelSetValue}/`)
          .then(response => {
            this.history[setting] = response.data;
          })
          .catch(error => {
      ...
      setControlPoint(record, setting) {
        this.controlPoints[setting] = { x: record.x, y: record.y };
        this.drawChart(setting);
      },
      getMttrIndex(mttr) {
